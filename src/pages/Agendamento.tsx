...
import { CalendarDays, RefreshCw, ChevronLeft, ChevronRight, User, FileText, Download, CheckCircle2 } from 'lucide-react'
...
function generateKitHTML(type: 'comprovante' | 'termo' | 'receita' | 'pre_op' | 'pos_op',
  patient: Patient, scheduledDate: string, scheduledTime: string, eye: string, conduct?: MedRecord['conduct']) {
  ...
  return (
    <PageLayout title="Agendamento" subtitle="Calendário cirúrgico e kit de agendamento"
      ...
      <div className="flex gap-6 h-[calc(100vh-145px)]">
        ...
        <div className="w-64 shrink-0 card flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-auto scrollbar-thin divide-y divide-slate-50">
            {loading ? (
              ...
              <button key={v.id} onClick={() => { setSelected(v); setSelectedDate(''); setConfirmed(false) }}
        ...
        {/* Main */}
        <div className="flex-1 flex flex-col gap-5 overflow-auto scrollbar-thin">
          {!selected ? (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      <button
                        key={type}
                        onClick={() => printDoc(
                          generateKitHTML(type, selected.patient, selectedDate, selectedTime, eye, conduct),
                          label
                        )}
                        className="flex items-center gap-3 p-4 rounded-xl border border-slate-200 hover:border-brand-300 hover:bg-brand-50 transition-colors text-left"
                      >
                    ...
                    >
                      <Download size={14} />
                      Imprimir Kit Completo (5 documentos)
                    </button>
                  </div>
                </div>
              )}
